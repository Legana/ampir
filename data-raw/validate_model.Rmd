...
#tree_grid <- expand.grid(mtry = c(34, 35, 36, 37, 39, 40),
                         #splitrule = "extratrees",
                        # min.node.size = c(0.5, 0.7, 0.9, 1, 1.2, 1.4))
...
ranger_bg60 <- train(Label~., data = featuresTrain[,-c(1,27:45)],
      method = "ranger",
      trControl = trctrl_prob,
      preProcess = c("center", "scale"),
      num.trees = 100,
...
test_pred <- predict(ranger_bg60, featuresTest)
confusionMatrix(test_pred, featuresTest$Label)

mcc(TP = 889, FP = 107, TN = 933, FN = 63)

test_pred_prob <- predict(ranger_bg60, featuresTest, type = "prob")
...
#plot(roc_out, print.thres="best", print.thres.best.method="closest.topleft")
...
ggroc(roc_out) + xlab("FPR") + ylab("TPR") + geom_segment(aes(x = 0, xend = 1, y = 0, yend = 1), color="darkgrey", linetype="dashed")
