#' Calculate the pseudo amino acid composition for peptides
#'
...
#'
calc_peptide_pseudo_comp <- function(seq,lambda = NULL) {
  ...
  # change min lamda to be a small value as peptides are shorter
  # lamba must be less than sequence length
  if (is.null(lambda)){
    max_lambda <- max(sapply(seq, nchar))
    lambda <- min(4, max_lambda)
  }
