...
randoms2_features <- calculate_features(randoms2$seq.aa)

randoms3_features <- calculate_features(randoms3$seq.aa)
```
...
```{r}
predict_AMP_prob <- function(df, classify = FALSE) {
  #load model
  load("svmRadialwithprobamph.Rdata")
...
predict_AMP_prob(little_features, classify = TRUE)
```
...
pred_randoms <- predict(svmRadialwithprob_amph, randoms_features)
pred_randoms <- as.data.frame(pred_randoms)

pred_randoms$pred_randoms[grep('Tg', pred_randoms$pred_randoms)]
#5/100 Tg

pred_randoms2 <- predict(svmRadialwithprob_amph, randoms2_features)
grep('Tg', pred_randoms2)
#6/100 Tg

pred_randoms3 <- predict(svmRadialwithprob_amph, randoms3_features)
grep('Tg', pred_randoms3)
#4/100  Tg

little_probs <- predict_AMPs(little_features)
